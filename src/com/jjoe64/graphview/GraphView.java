...
abstract public class GraphView extends LinearLayout
{
	...
	private class GraphViewContentView extends View
	{
				...
				Canvas canvas)
		{
			...
			lineHeight = (int) ((float) textBounds.height());
			// draw each line
			for (int i = 0; i < lines.length; ++i)
			{
				canvas.drawText(lines[i], x, (float) (y + (0.6 * yoffset)),
						paint);
		...
		protected void onDraw(Canvas canvas)
		{
			...
			{
				paint.setTextSize(getGraphViewStyle().getTextSize() * 2);
			...
			for (int i = 0; i < horlabels.length; i++)
			{
				...
				drawMultilineText(horlabels[i], x,
						height - labelTextHeight + 4, paint, canvas);
			}
